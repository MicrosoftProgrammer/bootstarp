...
class TCPDF {
	...
	public function Header() {
		if ($this->header_xobjid === false) {
			...
			if (($_SESSION["Logo"] ) AND ($_SESSION["Logo"]  != K_BLANK_IMAGE)) {
				$imgtype = TCPDF_IMAGES::getImageFileType("../../images/".$_SESSION["Logo"]);
				if (($imgtype == 'eps') OR ($imgtype == 'ai')) {
					$this->ImageEps("../../images/".$_SESSION["Logo"], '', '',32);
				} elseif ($imgtype == 'svg') {
					$this->ImageSVG("../../images/".$_SESSION["Logo"], '', '', 32);
				} else {
					$this->Image("../../images/".$_SESSION["Logo"], '', '',32);
				}
				$imgy = $this->getImageRBY();
			} else {
				$imgy = $this->y;
			}
			$cell_height = $this->getCellHeight(20 / $this->k);
			// set starting margin for text data cell
			if ($this->getRTL()) {
				$header_x = $this->original_rMargin + (20 * 1.1);
			} else {
				$header_x = $this->original_lMargin + (20 * 1.1);
			}
			$cw = $this->w - $this->original_lMargin - $this->original_rMargin - (20 * 1.1);
			$this->SetTextColorArray(array(0,0,0));
			// header title
			$this->SetFont('helvetica', 'B',25);
			$this->SetX($header_x);
					$this->MultiCell($cw, $cell_height, '<b align="center" style="font-stretch:100%;">'.$_SESSION["CompanyName"].'</b>', 0, '', 0, 1, '', '', true, 0, true, true, 0, 'M', false);

		$this->SetFont('helvetica', 'B',10);
			$this->SetX($header_x);
			
			$this->MultiCell($cw, $cell_height, '<b align="center"  style="font-stretch:110%;">'.$_SESSION["Address"].'</b>', 0, '', 0, 1, '', '', true, 0, true, true, 0, 'M', false);
...
Putting 1 is equivalent to putting 0 and calling Ln() just after. Default value: 0.
	...
	protected function openHTMLTagHandler($dom, $key, $cell) {
		...
		$bordertags = array('blockquote','br','dd','dl','span','div','dt','h1','h2','h3','h4','h5','h6','hr','li','ol','p','pre','ul','tcpdf','table');
